library(rjags)
source('./scripts/PIT/draws_EFF.R') #saved in draws_posterior
...
log_weights_i <- function(data_i, draws, log = TRUE) {
  ...
  for(i in 1 : dim(draws)[1]){
    #print(i/3000*100)
    mu[i] <-  draws[i, 1] + draws[i, 9 + data_i$id_player] + # beta0 + b0 +
      draws[i, 27 + data_i$id_player] + draws[i, 5] * data_i$sex + # b0j +betaW
      draws[i, 3] * data_i$class + draws[i, 4] * data_i$home + # betaC + betaH
      (draws[i, 2] + draws[i, 18 + data_i$id_player]) * data_i$id_game
...
for(obs in 1 : size_data){
  ...
  for(i in 1 : size_pred){
    ...
    sigma <- draws_posterior[idx, 6]
    
    
    mu <-  draws_posterior[idx, 1] + draws_posterior[idx, 9 + Trier_data$id_player[obs]] + # beta0 + b0 +
      draws_posterior[i, 27 + Trier_data$id_player[obs]] + draws_posterior[i, 5] * Trier_data$sex[obs] + # b0j +betaW
      draws_posterior[i, 3] * Trier_data$class[obs] + draws_posterior[i, 4] * Trier_data$home[obs] + # betaC + betaH
      (draws_posterior[i, 2] + draws_posterior[i, 18 + Trier_data$id_player[obs]]) * Trier_data$id_game[obs]
...
PIT_EFF <- PIT
save(PIT_EFF, file = './results/PIT_EFF.RData')
