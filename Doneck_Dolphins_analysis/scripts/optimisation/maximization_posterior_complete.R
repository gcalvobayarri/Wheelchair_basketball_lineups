library(lpSolve)
source('./functions/model_prediction_type.R')

set.seed(9257)
predictions <- model_prediction_type(player = 1:9, type = 'winscore', match = 19)
# arreglar
posterior <- c()
...
summary_winscore <- summary(as.factor(posterior))
levels(as.factor(posterior))
sort(summary_winscore)
posterior_winscore <- posterior

save(summary_winscore, posterior_winscore, 
     file = './results/lineups_winscore.RData')
# save(summary_EFF, file = './results/lineups_EFF.RData')
# save(summary_winscore, file = './results/lineups_winscore.RData')
